...
export default function ProjectsClient({ userRole }: { userRole?: "admin" | "user" }) {
  ...
  const [editProjectUsers, setEditProjectUsers] = useState<string[]>([]);
  const [editUsersDropdownOpen, setEditUsersDropdownOpen] = useState(false);
  const editUsersDropdownRef = useRef<HTMLDivElement>(null);
  ...
  const updateProjectMutation = useMutation({
    mutationFn: (data: { projectId: string; projectName: string; projectDescription: string; assignedUsers: string[] }) =>
      projectsApi.updateProject(data.projectId, { projectName: data.projectName, projectDescription: data.projectDescription, assignedUsers: data.assignedUsers }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["projects"] });
  ...
  const toggleEditUser = (userId: string) => {
    ...
    // Prevent admin from selecting themselves
    if (isAdmin && userId === myUserId) {
      showToast("You cannot assign a project to yourself", "error");
      return;
    }

    setEditProjectUsers(prev =>
      prev.includes(userId) ? prev.filter(id => id !== userId) : [...prev, userId]
    );
  };
  ...
  const handleEditProject = (project: Project) => {
    ...
    setEditProjectUsers(project.assignedUsers?.map(u => u._id) || []);
  };
  ...
  const handleSaveEditProject = () => {
    ...
    updateProjectMutation.mutate({
      ...
      assignedUsers: editProjectUsers,
    });
  };
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (editUsersDropdownRef.current && !editUsersDropdownRef.current.contains(event.target as Node)) {
        setEditUsersDropdownOpen(false);
      }
  ...
  // Filter projects by selected team - must be before early return
  const filteredProjects = useMemo(() => {
    const projects = projectsData?.data || [];
    if (!selectedTeamFilter) {
      return projects;
    }
    return projects.filter((p: Project) => p.team?._id === selectedTeamFilter);
  }, [projectsData?.data, selectedTeamFilter]);
  ...
  return (
    <div className="space-y-0">
      ...
      <div className="border-b border-[var(--border-subtle)] pb-0 md:-mx-6 md:px-6 -mt-6 pt-4 bg-[var(--bg-canvas)]">
        <div className="flex items-center gap-2.5 mb-3 text-[var(--text-tertiary)] flex-wrap">
          <div className="w-5 h-5 rounded bg-green-600 text-white flex items-center justify-center flex-shrink-0">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              <path d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
            </svg>
          </div>
          <span className="font-semibold text-[var(--text-primary)] text-[14px] sm:text-[16px]">
          ...
          <div className="relative ml-auto" ref={filterRef}>
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-colors ml-2 ${selectedTeamFilter
            ...
            {isFilterOpen && (
              <div className="absolute top-full right-0 mt-2 w-48 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-canvas)] shadow-lg py-2 z-50 animate-fade-in max-h-[50vh] overflow-y-auto">
                <div className="px-3 py-1.5 border-b border-[var(--border-subtle)]">
                  <span className="text-[11px] font-semibold text-[var(--text-muted)] uppercase tracking-wider">Filter by Team</span>
                </div>
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-[12px] transition-colors ${!selectedTeamFilter
                    ? "bg-[var(--accent-light)] text-[var(--accent)] font-medium"
                    : "text-[var(--text-secondary)] hover:bg-[var(--bg-surface)]"
                    }`}
                >
                  <span className="flex items-center gap-2">
                    {!selectedTeamFilter && <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polyline points="20 6 9 17 4 12" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                    All Teams
                  </span>
                </button>
                ...
                {teams && teams.length > 0 ? (
                  teams.map((team) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-[12px] transition-colors flex items-center gap-2 ${selectedTeamFilter === team._id
                        ? "bg-[var(--accent-light)] text-[var(--accent)] font-medium"
                        : "text-[var(--text-secondary)] hover:bg-[var(--bg-surface)]"
                        }`}
                    >
                      {selectedTeamFilter === team._id && <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polyline points="20 6 9 17 4 12" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                      <span className="truncate">{team.teamName}</span>
                    </button>
                  ))
                ) : (
                  <div className="px-3 py-2 text-[12px] text-[var(--text-muted)]">No teams available</div>
                )}
              </div>
            )}
          </div>
      ...
      <div className="mt-4 border border-[var(--border-subtle)] rounded-lg bg-[var(--bg-canvas)] shadow-sm">
        <div className="overflow-x-auto">
          {/* Table Header */}
          <div className="grid grid-cols-[2fr_1.5fr_1.5fr_1fr] gap-3 items-center px-4 sm:px-6 py-2.5 border-b border-[var(--border-subtle)] bg-[var(--bg-canvas)] text-[11px] font-semibold text-[var(--text-muted)] uppercase tracking-wide min-w-[500px] sm:min-w-full">
              ...
              return (
                <div key={project._id}>
                  <div
                    ...
                    className="grid grid-cols-[2fr_1.5fr_1.5fr_1fr] gap-3 items-center px-4 sm:px-6 py-2.5 border-b border-[var(--border-subtle)] last:border-0 hover:bg-[var(--bg-surface)] group text-[12px] sm:text-[13px] bg-[var(--bg-canvas)] transition-colors cursor-pointer min-w-[500px] sm:min-w-full"
                  >
                    {/* Name */}
                  ...
                  {/* Expanded Detail */}
                  {isExpanded && (
                    <div className="bg-[var(--bg-surface)] border-b border-[var(--border-subtle)] px-4 sm:px-6 py-3 sm:py-4">
                      <div className="flex flex-col sm:flex-row gap-3 sm:gap-6 items-start justify-between">
                        <div className="flex-1 min-w-0 w-full">
                          <div className="flex flex-col sm:flex-row gap-3 sm:gap-6 items-start mb-3">
                            {/* Left: Project properties */}
                            <div className="flex-1 min-w-0 space-y-2 sm:space-y-2.5">
                              <h3 className="text-[13px] sm:text-[14px] font-semibold text-[var(--text-primary)] mb-2 sm:mb-3">{project.projectName}</h3>

                              {project.projectDescription && (
                                <div className="flex items-start gap-2 sm:gap-4 text-[12px] sm:text-[13px]">
                                  <span className="w-20 sm:w-24 text-[var(--text-muted)] flex items-center gap-2 flex-shrink-0">
                            ...
                            <div className="w-full sm:w-[260px] sm:flex-shrink-0 sm:border-l border-t sm:border-t-0 border-[var(--border-subtle)] pt-3 sm:pt-0 sm:pl-5">
                          ...
                          {isAdmin && (
                            <div className="flex items-center gap-2 pt-3 border-t border-[var(--border-subtle)]">
                              <button
                                ...
                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[12px] text-[var(--text-secondary)] hover:bg-[var(--bg-surface-2)] transition-colors"
                              >
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                                Edit
                              </button>
                              <button
                                onClick={() => setDeleteConfirmProjectId(project._id)}
                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[12px] text-red-600 hover:bg-red-50 transition-colors"
                              >
          ...
          {isAdmin && (
            isAdding ? (
              <div className="grid grid-cols-[2fr_1.5fr_1.5fr_1fr] gap-3 items-start px-4 sm:px-6 py-3 border-t border-[var(--border-subtle)] bg-[var(--bg-canvas)] rounded-b-lg min-w-[500px] sm:min-w-full">
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 animate-toast-in" role="alert">
          <div className={`px-4 py-3 rounded-lg text-[12px] font-medium text-white shadow-lg ${toast.type === 'success' ? 'bg-emerald-950' : 'bg-red-900'}`}>
            {toast.message}
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-[var(--bg-canvas)] rounded-lg border border-[var(--border-subtle)] shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto animate-fade-in">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="text-[12px] font-semibold text-[var(--text-muted)] uppercase tracking-wider block mb-2">Project Name</label>
                <input
                  type="text"
                  value={editProjectName}
                  onChange={(e) => setEditProjectName(e.target.value)}
                  className="w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-md px-3 py-2 text-[13px] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Enter project name"
                />
              </div>

              <div>
                <label className="text-[12px] font-semibold text-[var(--text-muted)] uppercase tracking-wider block mb-2">Description</label>
                <textarea
                  value={editProjectDesc}
                  onChange={(e) => setEditProjectDesc(e.target.value)}
                  className="w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-md px-3 py-2 text-[13px] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
              ...
              <div>
                <label className="text-[12px] font-semibold text-[var(--text-muted)] uppercase tracking-wider block mb-2">Assigned Users</label>
                <div className="relative" ref={editUsersDropdownRef}>
                  <button
                    onClick={() => setEditUsersDropdownOpen(!editUsersDropdownOpen)}
                    className="w-full flex items-center gap-2 bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-md px-3 py-2 text-[12px] text-[var(--text-secondary)] hover:bg-[var(--bg-surface-2)] transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                    <span className="flex-1 text-left">
                      {editProjectUsers.length > 0 ? `${editProjectUsers.length} user${editProjectUsers.length !== 1 ? 's' : ''} selected` : 'Select users'}
                    </span>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9" /></svg>
                  </button>

                  {editUsersDropdownOpen && (
                    <div className="absolute z-50 top-full left-0 mt-1 w-full rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-canvas)] shadow-lg py-1 max-h-48 overflow-y-auto">
                      {users.length === 0 ? (
                        <div className="px-3 py-2 text-[12px] text-[var(--text-muted)]">No users available</div>
                      ) : (
                        users.map(u => (
                          <div
                            key={u._id}
                            onClick={() => toggleEditUser(u._id)}
                            className={`flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-[var(--bg-surface)] transition-colors ${editProjectUsers.includes(u._id) ? "bg-blue-50" : ""}`}
                          >
                            <div className={`w-3 h-3 rounded border flex items-center justify-center flex-shrink-0 ${editProjectUsers.includes(u._id) ? "bg-blue-500 border-blue-500 text-white" : "border-gray-300"}`}>
                              {editProjectUsers.includes(u._id) && <svg width="8" height="6" fill="currentColor" viewBox="0 0 8 6"><path d="M1 3 L3 5 L7 1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none" /></svg>}
                            </div>
                            <span className="text-[12px] text-[var(--text-primary)]">{u.firstName} {u.lastName}</span>
                          </div>
                        ))
                      )}
                    </div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-[var(--border-subtle)] bg-[var(--bg-surface-2)]">
              <button
                onClick={() => setEditingProjectId(null)}
                className="px-4 py-2 text-[12px] font-medium text-[var(--text-secondary)] hover:bg-[var(--bg-surface)] rounded-md transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-2 text-[12px] font-medium text-white bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 rounded-md transition-colors"
              >
          ...
          <div className="bg-[var(--bg-canvas)] rounded-lg border border-[var(--border-subtle)] shadow-xl max-w-sm w-full animate-fade-in">
            ...
            <div className="p-6">
              ...
              <div className="bg-red-50 border border-red-200 rounded-md px-3 py-2 mt-4">
                <p className="text-[12px] font-semibold text-red-700">
                  {filteredProjects.find(p => p._id === deleteConfirmProjectId)?.projectName}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-[var(--border-subtle)] bg-[var(--bg-surface-2)]">
              ...
              <button
                ...
                className="px-4 py-2 text-[12px] font-medium text-white bg-red-600 hover:bg-red-700 disabled:bg-gray-400 rounded-md transition-colors"
              >
