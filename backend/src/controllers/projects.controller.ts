...
export class ProjectsController {
    ...
    async updateProject(req: Request, res: Response): Promise<void> {
        try {
            ...
            const result = await this.projectsService.updateProject(id, body, userId);
            res.json(result);
        } catch (error) {
     ...
     */
    async getProjectsForUser(req: Request, res: Response): Promise<void> {
        try {
            const userId = req.user?.userId;
            if (!userId) {
                res.status(401).json({ error: "Unauthorized" });
            ...
            const result = await this.projectsService.getProjectsForUser(targetUserId, userId);
            res.json(result);
        } catch (error) {
            console.error("Get projects for user error:", error);
            if (error instanceof Error) {
                if (error.message.includes("admin")) {
                    res.status(403).json({ error: error.message });
                } else if (error.message.includes("not found")) {
